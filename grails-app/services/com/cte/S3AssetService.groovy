...
class S3AssetService {
  ...
  def createTempFilesOfMultipartsFiles(document,type) {
    ...
    def amazonResponse = amazonWebService.s3.putObject(new PutObjectRequest(s3AsseetProperties.bucket, "${s3AsseetProperties.title}.${s3AsseetProperties.mimeType}",file).withCannedAcl(CannedAccessControlList.PublicRead))
  ...
  private def createElenmetsOfS3Asset(file,type) {
    ...
    [title:hash,mimeType:extention,bucket:grailsApplication.config.grails.plugin.awssdk.bucketName,localPath:grailsApplication.config.grails.plugin.awssdk.url.content,originalName:name,type:type]
  }
  ...
  private def saveS3Asset(properties) {
    ...
    s3Asset.localUrl = "${s3Asset.protocol}${properties.bucket}.${properties.localPath}${properties.title}.${properties.mimeType}"
